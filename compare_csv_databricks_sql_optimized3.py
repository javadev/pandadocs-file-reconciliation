...
def compare_csv_files_no_full_outer(file1: str, file2: str, key_columns: list, other_columns: list):
    ...
    # SQL Query without FULL OUTER JOIN
    combined_sql = f"""
    SELECT
        {", ".join([f"COALESCE(table1.{col}, table2.{col}) AS {col}" for col in key_columns])} AS key,
        CASE
            WHEN {keys_condition} AND {" AND ".join([f"table1.{col} IS NULL AND table2.{col} IS NOT NULL" for col in other_columns])} THEN 'New'
            WHEN {keys_condition} AND {" AND ".join([f"table1.{col} IS NOT NULL AND table2.{col} IS NULL" for col in other_columns])} THEN 'Missed'
            WHEN {keys_condition} AND {" AND ".join([f"table1.{col} != table2.{col}" for col in other_columns])} THEN 'Changed'
        END AS type,
    ...
    FROM
        ...
        ON {keys_condition}
    WHERE
        {" AND ".join([f"table2.{col} IS NULL" for col in other_columns])}
    ...
    FROM
        ...
        ON {keys_condition}
    WHERE
        {" AND ".join([f"table1.{col} IS NULL" for col in other_columns])}
    """
    ...
    result_df.write.mode("overwrite").option("header", "true").csv("comparison_results_no_full_outer.csv")
...
if __name__ == '__main__':
    ...
    compare_csv_files_no_full_outer(file1, file2, key_columns, other_columns)
